...
iPackage=$(expr $iPackage + 1)
  ...
  packageAtLevel[$iPackage]=0
    testPresence[$iPackage]="echo \"program dummy; end program\" > dummy.F90; gfortran dummy.F90 $moduleDirs -lfgsl_gfortran"
...
buildEnvironment[$iPackage]=""
   ...
   configOptions[$iPackage]="skip"
        makeTest[$iPackage]="-f Makefile-libbz2_so"
    ...
    if [ ${packageAtLevel[$i]} -le $installLevel ]; then
		...
		if [ $isPython -eq 1 ]; then
			...
			exit 1
                    ...
                    # Hardwired magic.
                    # For bzip2 we have to install shared libraries manually......
		    if [ $i -eq $iBZIP2 ]; then
 			if [ $installAsRoot -eq 1 ]; then
