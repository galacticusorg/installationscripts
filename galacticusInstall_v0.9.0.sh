...
if [ ! -e $galacticusInstallPath ]; then
    ...
    hg clone https://abensonca@bitbucket.org/abensonca/galacticus_v0.9.0 $galacticusInstallPath/v0.9.0
    if [ $? -ne 0 ]; then
...
if [ "$RESPONSE" = yes ] ; then
	...
	awk 'BEGIN {inGLC=0} {if (index($0,"Alias to configure the environment to compile and run Galacticus v0.9.0") > 0) inGLC=1;if (inGLC == 0) print $0; if (inGLC == 1 && index($0,"'"'"'")) inGLC=0}' $HOME/.bashrc > $HOME/.bashrc.tmp
...
if [ "$RESPONSE" = yes ] ; then
	...
	awk 'BEGIN {inGLC=0} {if (index($0,"Alias to configure the environment to compile and run Galacticus v0.9.0") > 0) inGLC=1;if (inGLC == 0) print $0; if (inGLC == 1 && index($0,"'"'"'")) inGLC=0}' $HOME/.cshrc > $HOME/.cshrc.tmp
